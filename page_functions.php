...
// debug:
// echo '<h1>PAGE_FUNCTIONS Called successfully</h1>';
...
13	  MC		creator_drop_down($this_user_ID, $form_element_name = 'created_by')		Show list of users with preselected (opt.)			
14	  MC		supplier_drop_down($this_sup_ID, $form_element_name = 'sup_ID')			Show list of suppliers with preselect (opt.)		
...
32	  MC		location_drop_down($this_location_ID, $form_element_name = 'location_ID')		Show list of locations with preselected (opt.)	
...
/* ****************************************************************** */
	...
	function pagehead($page_id = 0, $record_id=NULL) {

	// debug:
	// echo '<h1>PAGE_HEAD function started</h1>';
	...
	while($row_get_page = mysqli_fetch_array($result_get_page)) {
			...
			$GLOBALS['table_name'] 			= $page_lookup_table;
			$GLOBALS['page_og_type'] 		= $page_og_type;				// hoping to use this data in the footer also
			$GLOBALS['page_name_EN']		= $page_name_EN;
			$GLOBALS['page_name_CN']		= $page_name_CN;
			
			if ( ($page_parent_ID != 0) && ($page_parent_ID != '') ) {
			
				// let's get the parent page info also:
				
				$get_p_page_SQL = "SELECT * FROM `pages` WHERE `ID` = '" . $page_parent_ID . "'";
				// echo '<h1>' . $get_p_page_SQL . '</h1>';

				$result_get_p_page = mysqli_query($con,$get_p_page_SQL);
				$p_pages_found = 0;
				// while loop
				while($row_get_p_page = mysqli_fetch_array($result_get_p_page)) {

						// set vars:
						$p_page_id 					= $row_get_p_page['ID'];
						$p_page_name_EN 			= $row_get_p_page['name_EN'];
						$p_page_name_CN 			= $row_get_p_page['name_CN'];
						$p_page_parent_ID 			= $row_get_p_page['parent_ID'];
						$p_page_dept_ID 			= $row_get_p_page['dept_ID'];
						$p_page_main_menu 			= $row_get_p_page['main_menu'];
						$p_page_footer_menu 		= $row_get_p_page['footer_menu'];
						$p_page_filename 			= $row_get_p_page['filename'];
						$p_page_icon 				= $row_get_p_page['icon'];
						$p_page_privacy 			= $row_get_p_page['privacy'];
						$p_page_min_user_level 		= $row_get_p_page['min_user_level'];
						$p_page_created_by 			= $row_get_p_page['created_by'];
						$p_page_date_created 		= $row_get_p_page['date_created'];
						$p_page_status 				= $row_get_p_page['status'];
						$p_page_order 				= $row_get_p_page['order'];
						$p_page_og_locale 			= $row_get_p_page['og_locale'];
						$p_page_og_type 			= $row_get_p_page['og_type'];
						$p_page_og_desc 			= $row_get_p_page['og_desc'];
						$p_page_og_section 			= $row_get_p_page['og_section'];
						$p_page_side_bar_config 	= $row_get_p_page['side_bar_config'];
						$p_page_lookup_table 		= $row_get_p_page['lookup_table']; // look up info for TITLE tag!

						$GLOBALS['p_page_id'] 				= $p_page_id; 					// hoping to use this data in the footer also
						$GLOBALS['p_page_lookup_table'] 	= $p_page_lookup_table; 			// hoping to use this data in the footer also
						$GLOBALS['p_page_og_type'] 			= $p_page_og_type;				// hoping to use this data in the footer also
						$GLOBALS['p_page_name_EN']			= $p_page_name_EN;
						$GLOBALS['p_page_name_CN']			= $p_page_name_CN;
...
-->
	...
	<?
		...
		<div class="rotate bootstro" style="position: fixed; right: -30px; top: 50%; z-index: 9999; margin-top: -100px;"
			...
			data-bootstro-width='400px'
			 ...
			 data-boostro-html='true'><a class="btn btn-warning simple-ajax-modal" href="info_pop.php?id=feedback&referrer=<?php echo $_SERVER['HTTP_REFERER']; ?>&ref_page=<?php echo $page_filename; ?>&page_id=<?php echo $page_id; ?>">Feedback / 反馈</a>
		</div>
		<div class="rotate bootstro" style="position: fixed; right: 13px; top: 50%; z-index: 9999; margin-top: -16px;"
			data-bootstro-title="Admin Options!"
			data-bootstro-content='Here you can make changes to the page'
			data-bootstro-placement='left'
			data-bootstro-width='400px'
			 data-bootstro-step='8'
			 data-boostro-html='true'><a class="btn btn-default simple-ajax-modal" href="info_pop.php?id=admin&referrer=<?php echo $_SERVER['HTTP_REFERER']; ?>&ref_page=<?php echo $page_filename; ?>&page_id=<?php echo $page_id; ?>">&nbsp;<i class="fa fa-cog"></i>&nbsp;</a>
		</div>
	<?php
	...
	// NOW START THE PAGE WITH BREDCRUMB NAV AND PAGE TITLE:
...
<section role="main" class="content-body">
    <header class="page-header">
        ...
        <div class="right-wrapper pull-right">
        	<?php 
        	// THIS SHOULD BE DYNAMICALLY BUILT ACCORDING TO THE RELATIONSHIP / HEIRARCHY IN THE DB
        	?>
            <ol class="breadcrumbs">
                ...
                <li>
                  <a href="<?php 
                  	...
                  	echo $p_page_filename; 
                	
                	// if we are looking at an EDIT page, let's include the ID number to redirect to the profile:
                	if ($page_og_type == 'edit') {
                    ...
                    <span><?php 
                	echo $p_page_name_EN; 
                	if (($p_page_name_CN !='')&&($p_page_name_CN!='中文名')) { 
                		echo ' / ' . $p_page_name_CN; 
                	}
...
<!-- : END MAIN PAGE BODY in pagefoot function -->
...
<!-- Examples -->
		...
		$random_image_num = rand(1, 6);
		
		?>
		<img src="assets/images/other/header_0<?php echo $random_image_num; ?>.jpg" class="img-fluid" alt="Responsive image" alt="image not found" style="height: 75%;" />
		<?php
					...
					<div class="row">
						...
						?>
							<div class="alert alert-success">
								...
								if ($_REQUEST['action'] == 'add_results') { ?>
									<!-- ADD NEW LINE ITEM -->
									<span class="fa-stack fa-3x">
										<i class="fa fa-circle-o fa-stack-2x"></i>
										<i class="fa fa-check fa-stack-1x"></i>
									</span>
									<strong>Well done!</strong> You successfully added the results to this report.
								<?php 
								}
...
function inspection_method_class($inspection_method_class_id, $profile_link = 0) {
	...
	if ($inspection_method_class_id == 0) {
		?>
		<span class="btn btn-danger">
			<i class="fa fa-exclamation-triangle"></i>
			NO CLASS FOUND!
		...
		// now get the part info:
		$get_inspection_method_class_SQL = "SELECT * FROM `inspection_method_class` WHERE `ID` = " . $inspection_method_class_id;
		// echo $get_parts_SQL;

		$result_get_inspection_method_class = mysqli_query($con,$get_inspection_method_class_SQL);

		// while loop
		while($row_get_inspection_method_class = mysqli_fetch_array($result_get_inspection_method_class)) {
			$inspection_method_class_ID 			= $row_get_inspection_method_class['ID'];
			$inspection_method_class_name_EN 		= $row_get_inspection_method_class['name_EN'];
			$inspection_method_class_name_CN 		= $row_get_inspection_method_class['name_CN'];
			$inspection_method_class_record_status 	= $row_get_inspection_method_class['record_status'];
			$inspection_method_class_AQL_level 		= $row_get_inspection_method_class['AQL_level'];
			$inspection_method_class_sample_level 	= $row_get_inspection_method_class['sample_level'];
			...
			if ($profile_link == 1) {
				?>
				<a href="inspection_method_class.php?id=<?php echo $inspection_method_class_ID; ?>" title="Click here to view more details">
				<?php
				$close_link = "</a>";
			}

			echo $inspection_method_class_name_EN;
			if (($inspection_method_class_name_CN != '') && ($inspection_method_class_name_CN != '中文名')) { echo ' / ' . $inspection_method_class_name_CN; }
			echo $close_link; // maybe blank - see above
...
function inspection_method_class_drop_down($this_inspection_method_class_ID, $form_element_name = 'method_class_ID') {
	...
	<select data-plugin-selectTwo class="form-control populate" name="<?php echo $form_element_name; ?>" id="<?php echo $form_element_name; ?>">
		...
		$get_inspection_method_class_SQL = "SELECT * FROM `inspection_method_class` WHERE `record_status` = 2";
		// echo $get_loc_SQL;

		$result_get_inspection_method_class = mysqli_query($con,$get_inspection_method_class_SQL);

		// while loop
		while($row_get_inspection_method_class = mysqli_fetch_array($result_get_loc)) {
			$inspection_method_class_id 			= $row_get_inspection_method_class['ID'];
			$inspection_method_class_name_EN 		= $row_get_inspection_method_class['name_EN'];
			$inspection_method_class_name_CN 		= $row_get_inspection_method_class['name_CN'];
			$inspection_method_class_record_status 	= $row_get_inspection_method_class['record_status'];
			...
			// now print the result
				...
				?>
				<option value="<?php echo $inspection_method_class_id; ?>" <?php if ($inspection_method_class_id == $this_inspection_method_class_ID) { ?> selected="selected"<?php } ?>>
					<?php echo $inspection_method_class_name_EN; if (($inspection_method_class_name_CN != '') && ($inspection_method_class_name_CN != '中文名')) { echo " / " . $inspection_method_class_name_CN; } ?>
				</option>
				<?php
			}
			?>
...
function form_buttons($cancel_url, $record_id = 0, $add_VARS = '') { 
	...
	if ($part_id == 0) {
		...
		while($row_get_part = mysqli_fetch_array($result_get_part)) {
			...
			if ($show_button == 1) { 
				...
				// removed class 'btn btn-info btn-xs'
	
				?><a href="part_view.php?id=<?php echo $part_id; ?>" class="" title="View <?php echo $part_name_EN; 
		...
		<?php
			...
			$result_get_part_rev = mysqli_query($con,$get_part_rev_SQL);
				...
				if ($show_button == 1) { 
					...
					// removed btn btn-xs btn-<?php echo $button_style;
					?><a href="part_view.php?id=<?php echo $part_rev_part_ID; ?>&rev_id=<?php echo $part_rev_ID; ?>" class="" title="Rev. #: <?php echo $part_rev_ID; ?> (Part #: <?php echo $part_rev_part_ID; ?>)"><?php 
				} 
				else { 
					?>Rev. <?php
					$close_link = '';
	...
	if ($part_id == 0) {
		...
		while($row_get_part = mysqli_fetch_array($result_get_part)) {
			...
			if ($show_button == 1) { 
				...
				$close_link = '</a>';
				// removed class 'btn btn-default btn-xs'
				?><a href="part_view.php?id=<?php echo $part_id; ?>" class="" title="View <?php echo $part_name_EN; 
				if (($part_name_CN!='')&&($part_name_CN!='中文名')) { echo ' / ' . $part_name_CN; } ?> Part Profile"><?php 
			} 
...
function warehouse_drop_down($lookup_ID, $form_element_name = 'location_ID') {
