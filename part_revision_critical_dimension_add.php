...
?>
    ...
    <div class="row">
		...
		if ( (!isset($_REQUEST['inspection_method_ID'])) && ($record_id == '') ) {
				...
				<div class="col-md-5">
					...
					<select onChange="document.location = this.value" data-plugin-selectTwo class="form-control populate" name="inspection_method_ID_jump">
						...
						?>">
							...
							?>
							<option value="part_revision_critical_dimension_add.php?part_rev_id=<?php echo $crit_dim_part_revision_ID; ?>&inspection_method_ID=<?php echo $inspection_method_ID; ?>&id=<?php echo $record_id; ?>"><?php 
			...
			if ($crit_dim_inspection_method_ID!='') {
				$inspection_method_ID = $crit_dim_inspection_method_ID;
			}
			else {
				$inspection_method_ID = $_REQUEST['inspection_method_ID'];
		...
		if ( ( isset($_REQUEST['inspection_method_ID']) ) || ($form_action == 'EDIT') ) {
			// ASK FOR THE TABLE
			// echo '<h1>No Table</h1>'; 
