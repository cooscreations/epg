...
while($row_get_user = mysqli_fetch_array($result)) {
	...
	$user_status 			= $row_get_user['record_status'];

} // end get user info WHILE loop

if ($user_status != '2') { // USER IS PRESENT BUT DELETED
	if ($user_status == '1') { $status = 'Pending'; }
	else { $status = 'Deleted'; }
	header("Location: login.php?msg=NG&error=invalid_login&status=" . $status . "");
}
else { // USER IS PRESENT AND ACTIVE:
